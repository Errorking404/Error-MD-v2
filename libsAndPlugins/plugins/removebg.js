...
KEY LINK : https://www.remove.bg/dashboard#api-key\`\`\``
    ...
    if (!message.reply_message || !message.reply_message.image)
      return await message.reply(Lang.NEED_PHOTO)

    let location = await message.reply_message.downloadMediaMessage()
    let buffer = await removeBg(location, config.REMOVEBG)

    if (typeof buffer == "string") {
      if (buffer.includes("403")) return await message.reply(Lang.RBGING)
      else if (buffer.includes("402")) return await message.reply(Lang.LIMIT)
      else return await message.reply(buffer)
    }
